-- Langkah 1: Cipta Jadual "profiles"
...
CREATE TABLE public.profiles (
  ...
  role text NULL, -- Peranan pengguna ('user' atau 'admin')
  ...
  payment_status text NULL DEFAULT 'pending'::text, -- Status pembayaran ('pending', 'paid', 'rejected')
  payment_reference text NULL, -- Rujukan pembayaran manual dari pengguna
  toyyibpay_bill_code text NULL, -- Kod bil dari ToyyibPay untuk rujukan (jika masih diguna)
  created_at timestamptz NULL DEFAULT now(), -- Tarikh dan masa rekod dicipta
  
  CONSTRAINT profiles_pkey PRIMARY KEY (id), -- Tetapkan 'id' sebagai Kunci Primer
  CONSTRAINT profiles_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE -- Hubungkan 'user_id' ke jadual 'auth.users'
);

-- Memberi komen pada jadual untuk dokumentasi
COMMENT ON TABLE public.profiles IS 'Menyimpan maklumat profil dan langganan pengguna.';
COMMENT ON COLUMN public.profiles.user_id IS 'Sambungan ke jadual auth.users';
