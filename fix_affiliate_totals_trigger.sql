...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='affiliates' AND column_name='total_sales') THEN
        ...
        RAISE NOTICE 'Column total_sales added to affiliates table.';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='affiliates' AND column_name='total_commission') THEN
        ...
        RAISE NOTICE 'Column total_commission added to affiliates table.';
    END IF;
END $$;
...
RAISE NOTICE 'Trigger function update_affiliate_totals created.';
...
RAISE NOTICE 'Trigger on_new_sale_update_affiliate_totals created and linked to sales table.';
